...
void arrInit(int board[][COLS])
{
    ...
    //Putting 15 zeros in random positions
    for(i = 0; i < nZeros; i++)
    {
        int x = rand() % ROWS;
        int y = rand() % COLS;
...
void floodFill(int board[][COLS], int x, int y, int oldColor, int newColor)
{
    if(x >= 0 && x < ROWS && y >= 0 && y < COLS && board[x][y] == oldColor && board[x][y] != newColor && board[x][y] != 0)
    {
        ...
        floodFill(board, x+1, y+1, oldColor, newColor); //south-east node(diagonal)
        floodFill(board, x-1, y-1, oldColor, newColor); //north-west node(diagonal)
        floodFill(board, x-1, y+1, oldColor, newColor); //north-east node(diagonal)
        floodFill(board, x+1, y-1, oldColor, newColor); //south-west node(diagonal)
    }
}
...
int chkBoard(int board[][COLS])
{
    ...
    //If there are zeros around the upper-left corner color or is zero then return ERROR
    if((board[0][1] == 0 && board[1][0] == 0 && board[1][1] == 0) || color == 0) return ERROR;
    
    //Checks if the whole board is filled with the same color (except zero)
    for(i = 0; i < ROWS; i++)
        for(j = 0; j < COLS; j++)
        {
            if(board[i][j] != 0)
...
int nColor(int board[][COLS], int lastNum1, int lastNum2, int lastNum3)
{
    ...
    do
    {
        if(remNColors == 5)
        {
            ...
            else if(board[0][0] != board[1][0] && board[1][0] != 0)
            {
            ...
            else if(board[0][0] != board[1][1] && board[1][1] != 0)
            {
...
int main(int argc, const char * argv[])
{
    ...
    if((win = chkBoard(board)) == ERROR) color = 0; //if the board has missplaced zeros then the game stops with an error
    ...
    putchar('\n');
    if(win == TRUE) printf("Computer finished the game in %d moves!\n", nMoves);
    else if(win == ERROR) printf("The game can't be played since there are one or more zeros at the beginning. Please start a new game!\n");
    putchar('\n');
