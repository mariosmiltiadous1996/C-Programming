...
void svStats(char *cWords[], int *histo, int *nCWords);
    ...
    while (choice)
    {
        ...
        switch(choice)
        {
            ...
            case 3:
                ...
                break;
            case 4:
                if(cWords == NULL || histo == NULL)
                {
                    cWords = cntCWords(fl, &nCWords);
                    histo = histogram(fl, cWords, &nCWords, 0);
                }
                
                svStats(cWords, histo, &nCWords);
                system("cls");
                printf("\nStatistics succesfully saved in the same directory as the program.\n\n");
...
void svStats(char *cWords[], int *histo, int *nCWords)
{
    ...
    printf("\nPlease type the name of the file you want to save the stats including the extension <<.txt>>: ");
    scanf("%[^\n]s%*c", s_fl);
    
    if((fp = fopen(s_fl, "wt")) == NULL)
    {
        puts("ERROR 5: Can't write data to file. Exiting...");
        system("timeout 1 > NUL");
        exit(EXIT_FAILURE);
    }
    
    fprintf(fp, "Number of C words in file: %d\n", *nCWords);
    fprintf(fp, "\n----------------------------------------------------------\n");
    fprintf(fp, "%5s%*s%*s%*s%*s\n", "Num", 3, "|", 8, "Words", 3, "|", 23, "Histogram");
    fprintf(fp, "----------------------------------------------------------\n");
    
    for(i = 0; i < *nCWords; i++)
    {
        fprintf(fp, "%4d%*s%*s%*s%*s ", histo[i], 4, "|", 5+(int)strlen(cWords[i])/2,cWords[i],4-(int)strlen(cWords[i])/2,"", 2, "|");
        for(j = 0; j < histo[i]; j++)
            fprintf(fp, "*");
        fprintf(fp, "\n");
    }
